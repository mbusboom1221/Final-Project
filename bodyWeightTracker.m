...
function [] = bodyWeightTracker()
    ...
    %Displays text above Weight user input box
    ...
    %Button to clear all the user input boxes for ease of use after error
    %mesage dipslay
    ...
    %Text label for Calories In box
    ...
    %Test label for Calories Out Box
    ...
    %Slider that initializes trend line plotting and user selected goal
    %rate of weight loss for nutrition suggestions and trendline comparison
    ...
    %Labels upper threshold for slider as "Aggressive (-1.5%BW/Week)"
    gui.labelSliderHigh = uicontrol('style','text','string','Aggressive (-1.5%BW/week)','units','normalized','position',[.05 .5 .2 .05]);
    %Labels lower threshold for slider as "Aggressive (-0.5%BW/Week)"
    gui.labelSliderLow = uicontrol('style','text','string','Conservative (-0.5%BW/week)','units','normalized','position',[.05 .0001 .2 .05]);
    %Button at top right of GUI to clear the figure and rerun the main
    %function
    gui.resetButton = uicontrol('style','pushbutton','string','Reset','units','normalized','position',[.75 .9 .2 .05],'callback',@resetGUI);
    %Buttton to calculate user's average caloric deficit
    ...
    %Text display the calculated average caloric deficit
    gui.avgDeficitDisplay= uicontrol('style','text','units','normalized','position',[.32 .025 .2 .05]);
    %Button to calculate user's average rate of actual weight loss
    ...
    %Text display the calculated user's average rate of actual weight loss
    gui.avgRateLossDisplay= uicontrol('style','text','units','normalized','position',[.525 .025 .2 .05]);
    %Button to calculate caloric intake goal for user based on selected
    %goal rate of weight loss with slider position
    ...
    %Text display the calculated caloric intake goal
